...
# 1. Load 2-Channel Data (original SG window=15)
...
print(f"Starting Experiment 8: 3-Seed Multi-Ensemble of Exp 2A (30 models total)...")
...
for seed in seeds:
    ...
    folds = get_kfold_indices(len(X), k=K, seed=seed)
    seed_val_rmses = []
    
    for fold in range(K):
        print(f"\n--- Seed {seed} | Fold {fold+1}/{K} ---")
        ...
        best_weights_path = f"best_model_exp8_seed{seed}_fold{fold}.pth"
        ...
        seed_val_rmses.append(best_val_rmse)
    ...
    mean_seed_rmse = np.mean(seed_val_rmses)
    all_seed_rmses.append(mean_seed_rmse)
    print(f"Seed {seed} Completed. Average CV RMSE: {mean_seed_rmse:.4f}")

# Final Summary
print(f"\n==========================================")
print(f"Experiment 8 CV Summary:")
print(f"Average CV RMSE across seeds: {np.mean(all_seed_rmses):.4f} ± {np.std(all_seed_rmses):.4f}")
print(f"==========================================")

# Ensemble average over all 30 models
...
}).to_csv("submission_exp8.csv", index=False, header=False)
print("Saved predictions to submission_exp8.csv")
