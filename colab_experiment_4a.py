...
def get_3ch_features(X_raw):
    feats = []
    # Channel 0: Raw + SNV
    x0 = apply_snv(X_raw.copy())[:, wavenumber_mask]
    feats.append(x0)
    
    # Channel 1: 1st Derivative + SG + SNV
    x1 = savgol_filter(X_raw, window_length=15, polyorder=2, deriv=1, axis=-1)
    x1 = apply_snv(x1)[:, wavenumber_mask]
    feats.append(x1)
    
    # Channel 2: 2nd Derivative + SG + SNV
    x2 = savgol_filter(X_raw, window_length=15, polyorder=2, deriv=2, axis=-1)
...
print(f"Starting Experiment 4A: 3-Channel Masked + Mixup 0.8 (5-Fold GroupKFold CV)...")
fold_val_rmses = []
predictions_all = []
...
for fold in range(K):
    ...
    best_weights_path = f"best_model_exp4a_fold_{fold}.pth"
...
print(f"\n==========================================")
print(f"Experiment 4A (3-Channel Masked GroupKFold) CV Summary:")
...
}).to_csv("submission_exp4a.csv", index=False, header=False)
print("Saved predictions to submission_exp4a.csv")
