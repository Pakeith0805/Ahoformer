...
def get_2ch_features(X_raw):
    feats = []
    # Channel 0: 1st Derivative + SG(15) + SNV (use SG 15 to preserve sharpness)
...
# Mixup function for adversarial data (we only mix moisture targets, species labels remain unmixed or we disable mixup for species)
...
class AdversarialWoodDataset(Dataset):
...
# 2. Define Gradient Reversal Layer (GRL)
...
# 3. Define Adversarial Ahoformer
class AdversarialAhoformer(nn.Module):
    ...
    def forward(self, x, alpha=1.0):
        ...
        # Species prediction (reverse gradient to force species-invariance)
        reversed_features = grad_reverse(mean_output, alpha)
        preds_species = self.species_classifier(reversed_features)
...
for fold in range(5):
    ...
    train_dataset = AdversarialWoodDataset(X_prep[train_idx], y_log[train_idx], species_labels[train_idx])
    val_dataset = AdversarialWoodDataset(X_prep[val_idx], y_log[val_idx], species_labels[val_idx])
    ...
    model = AdversarialAhoformer(base_model, num_species).to(device)
    
    # Loss functions: Huber Loss for moisture, CrossEntropy for species
    criterion_moisture = nn.HuberLoss(delta=0.1) # Bounded gradient for outliers
    ...
    for epoch in range(max_epochs):
        ...
        # Calculate adversarial scale lambda
        p = float(epoch) / max_epochs
        alpha_grl = 2.0 / (1.0 + np.exp(-10.0 * p)) - 1.0
        ...
        # Evaluate on validation fold (unseen species)
        model.eval()
        val_preds_log_list = []
        with torch.no_grad():
        ...
        # Record validation loss (Log MSE) for this epoch
        val_loss_log = np.mean((val_preds_log - y_log[val_idx]) ** 2)
        fold_epoch_losses[fold, epoch] = val_loss_log

# Find the epoch that minimizes average validation loss across folds
...
full_train_dataset = AdversarialWoodDataset(X_prep, y_log, species_labels)
...
final_model = AdversarialAhoformer(base_model, num_species).to(device)
...
for epoch in range(optimal_epoch):
    ...
    for batch_data in full_train_loader:
        ...
        if np.random.rand() < mixup_prob:
            bx_mixed, by_mixed, b_sp1, b_sp2, lam = mixup_data(bx, by, b_species, alpha=alpha_mixup)
            preds_m, preds_sp = final_model(bx_mixed, alpha_grl)
            loss_m = criterion_moisture(preds_m, by_mixed)
            loss_sp = lam * criterion_species(preds_sp, b_sp1) + (1 - lam) * criterion_species(preds_sp, b_sp2)
        else:
            preds_m, preds_sp = final_model(bx, alpha_grl)
            loss_m = criterion_moisture(preds_m, by)
            loss_sp = criterion_species(preds_sp, b_species)
    ...
    if (epoch + 1) % 10 == 0 or epoch == optimal_epoch - 1:
        print(f"Epoch {epoch+1:2d}/{optimal_epoch} | Moisture Loss: {train_loss_m:.4f} | Species Loss (Adv): {train_loss_sp:.4f} | GRL Lambda: {alpha_grl:.4f}")
...
test_dataset = AdversarialWoodDataset(X_test_prep)
...
}).to_csv("submission_adversarial.csv", index=False, header=False)

print("Single Adversarial Model predictions saved to submission_adversarial.csv")
