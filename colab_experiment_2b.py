...
class AugmentWoodDataset(Dataset):
    """
    Dataset wrapper that adds random wavelength shifting (jitter) on the fly during training.
    """
    ...
    def __getitem__(self, idx):
        x = self.x[idx] # (C, num_features)
        
        if self.augment:
            x = x.clone()
            
            # Wavelength Jitter (shifting spectra along the last axis)
            # Randomly shift by -2 to 2 steps with 50% probability
...
print(f"Starting Experiment 2B: 2-Channel + Wavelength Jitter (10-Fold CV)...")
...
for fold in range(K):
    ...
    best_weights_path = f"best_model_exp2b_fold_{fold}.pth"
...
print(f"\n==========================================")
print(f"Experiment 2B (Wavelength Jitter) CV Summary:")
...
}).to_csv("submission_exp2b.csv", index=False, header=False)
print("Saved predictions to submission_exp2b.csv")
