...
class Ahoformer(nn.Module): # modelを呼び出すと、initで定義してforwardで実行までを自動でやってくれる。
    def __init(self, num_embeddings):
        super().__init__()
        # 埋め込み層
    ...
    def forward(self, x):
        embedded_vectors = self.embedding_layer(x).detach()
        attention_out = self.attention(embedded_vectors)
        return attention_out, embedded_vectors