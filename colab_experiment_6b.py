...
# Override config variables dynamically before importing models
import config
config.d_model = 64
config.d_ff = 128
config.num_head = 4
...
mixup_prob = 0.8
alpha = 0.4
num_layers = 1 # 1-Layer model

print(f"Starting Experiment 6B: Full-Spectrum 2-Channel + 1-Layer + Log Checkpoint + Augmentation (5-Fold GroupKFold CV)...")
fold_val_rmses = []
predictions_all = []
...
for fold in range(K):
    ...
    best_weights_path = f"best_model_exp6b_fold_{fold}.pth"
...
print(f"\n==========================================")
print(f"Experiment 6B CV Summary:")
...
}).to_csv("submission_exp6b.csv", index=False, header=False)
print("Saved predictions to submission_exp6b.csv")
