...
# 1. Load 2-Channel Data (Channel 0: 1st deriv, Channel 1: 2nd deriv)
...
print(f"Starting Experiment 2: 2-Channel Derivative Stacking (10-Fold CV)...")
folds = get_kfold_indices(len(X), k=K, seed=42)
fold_val_rmses = []
predictions_all = []

# Load test data with 2 channels
...
for fold in range(K):
    ...
    best_weights_path = f"best_model_exp2_fold_{fold}.pth"
...
print(f"\n==========================================")
print(f"Experiment 2 (2-Channel Derivative) CV Summary:")
...
}).to_csv("submission_exp2.csv", index=False, header=False)
print("Saved predictions to submission_exp2.csv")
