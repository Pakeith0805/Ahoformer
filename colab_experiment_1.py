...
# 1. Load Data
X, y, sample_ids, species_ids = dataset.load_train_data("train.csv", use_savgol=True, use_snv=True, use_msc=False, num_channels=1)
y_log = np.log1p(y)
...
# Custom loss function: MSE on original scale with clamping
class ClampedOriginalScaleLoss(nn.Module):
    def __init__(self, max_clamp=300.0):
        super().__init__()
        self.max_clamp = max_clamp
        
    def forward(self, pred_log, target_log):
        # target_log is log1p(y_orig)
        y_orig = torch.expm1(target_log)
        
        # pred_log can be large during initial training, so clamp before expm1
        pred_log_clamped = torch.clamp(pred_log, -5.0, 6.0) # expm1(6.0) ~ 402
        pred_orig = torch.expm1(pred_log_clamped)
        pred_orig = torch.clamp(pred_orig, 0.0, self.max_clamp)
        
        # Compute MSE directly on original scale
        loss = torch.mean((pred_orig - y_orig) ** 2)
        return loss
...
print(f"Starting Experiment 1: Original-Scale Loss (10-Fold CV)...")
folds = get_kfold_indices(len(X), k=K, seed=42)
fold_val_rmses = []
predictions_all = []

# Load test data
X_test, test_sample_numbers, test_species_ids = dataset.load_test_data("test.csv", use_savgol=True, use_snv=True, use_msc=False, num_channels=1)
...
for fold in range(K):
    ...
    if model.embedding_layer.in_channels != 1:
    ...
    criterion = ClampedOriginalScaleLoss()
    optimizer = optim.AdamW(model.parameters(), lr=learning_rate, weight_decay=weight_decay)
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs, eta_min=1e-6)
    
    best_val_rmse = float('inf')
    best_weights_path = f"best_model_exp1_fold_{fold}.pth"
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 20 == 0 or epoch == epochs - 1:
            print(f"    Epoch {epoch+1:2d}/{epochs} | Train Loss (Original MSE): {train_loss:.4f} | Val RMSE: {val_rmse:.4f} | Best Val: {best_val_rmse:.4f}")
            
    print(f"  Fold {fold+1} Best Val RMSE: {best_val_rmse:.4f}")
    fold_val_rmses.append(best_val_rmse)
    
    # Predict on test using best fold weights
...
print(f"\n==========================================")
print(f"Experiment 1 (Original-Scale Loss) CV Summary:")
...
}).to_csv("submission_exp1.csv", index=False, header=False)
print("Saved predictions to submission_exp1.csv")
