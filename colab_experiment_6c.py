...
mixup_prob = 1.0 # 100% Mixup
alpha = 0.4
num_layers = 2

print(f"Starting Experiment 6C: Full-Spectrum 2-Channel + Mixup 1.0 + Log Checkpoint + Augmentation (5-Fold GroupKFold CV)...")
fold_val_rmses = []
predictions_all = []
...
for fold in range(K):
    ...
    best_weights_path = f"best_model_exp6c_fold_{fold}.pth"
...
print(f"\n==========================================")
print(f"Experiment 6C CV Summary:")
...
}).to_csv("submission_exp6c.csv", index=False, header=False)
print("Saved predictions to submission_exp6c.csv")
