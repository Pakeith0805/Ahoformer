...
for fold in range(5):
    ...
    for epoch in range(max_epochs):
        ...
        # Secondary safety clamp for log-MSE calculation
        val_preds_log = np.clip(val_preds_log, np.log1p(0.01), np.log1p(300.0))
...
# ----------------- CRITICAL FIX: Prediction Clamping -----------------
# Prevent expm1 explosion from out-of-domain overshoot predictions
test_preds_log = np.clip(test_preds_log, np.log1p(0.01), np.log1p(300.0))
# ---------------------------------------------------------------------

test_preds_orig = np.expm1(test_preds_log)
test_preds_orig = np.clip(test_preds_orig, 0.01, 300.0)

pd.DataFrame({
    "sample_number": test_sample_numbers,
    "moisture_content": test_preds_orig
}).to_csv("submission_daa_clamped.csv", index=False, header=False)

print("Clamped DAA Model predictions saved to submission_daa_clamped.csv")
